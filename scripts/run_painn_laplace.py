...
class LitPaiNNModel(L.LightningModule):
    ...
    def __init__(
        ...
        # Laplace
        use_laplace: bool = False,  # New flag for Laplace
        ...
        **kwargs,  # Ignore additional arguments
        ...
        self.use_laplace = use_laplace  # Store the Laplace flag
        ...
        # Initialize Laplace buffers if enabled
        if self.use_laplace:
            param_vector = torch.nn.utils.parameters_to_vector(self.parameters())
            self.register_buffer('posterior_mean', torch.zeros_like(param_vector))
            self.register_buffer('hessian_diag', torch.ones_like(param_vector))  # Initialize with ones
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        preds = self.forward(batch)
        if self.use_laplace:
            self._restore_params()  # Restore original parameters
    ...
    def compute_hessian_diag(self):
        ...
        self.model.eval()
        # Initialize hessian_diag
        hessian_diag = torch.zeros_like(self.posterior_mean)
        # Use a small batch or the entire dataset to estimate the Hessian
        # Here, we'll use a single batch for simplicity
        # In practice, you might want to use multiple batches or the entire training set
        for batch in self.trainer.datamodule.train_dataloader():
            ...
            grads = torch.nn.utils.parameters_to_vector([p.grad for p in self.parameters()]).detach()
            hessian_diag += grads ** 2
            break  # Remove this if you want to accumulate over more batches
        # Average the hessian_diag
        hessian_diag /= len(self.trainer.datamodule.train_dataloader())
        self.hessian_diag = hessian_diag + 1e-6  # Add a small value for numerical stability
        # Set posterior mean as current parameters
        self.posterior_mean = torch.nn.utils.parameters_to_vector(self.parameters()).detach()
        print("Hessian diagonal computed for Laplace approximation.")
    
    def sample_from_posterior(self, scale=1.0):
        """Sample parameters from the Laplace posterior and restore original parameters after."""
        if not self.use_laplace:
            raise ValueError("Laplace approximation is not enabled.")
        if not hasattr(self, 'hessian_diag'):
            raise ValueError("Hessian diagonal not computed. Ensure that compute_hessian_diag is called after training.")

        # Save original parameters
        original_params = torch.nn.utils.parameters_to_vector(self.parameters()).detach()
        # Sample from the Gaussian posterior
        std = torch.sqrt(1.0 / self.hessian_diag)
        z = torch.randn_like(self.posterior_mean)
        sampled_params = self.posterior_mean + scale * z * std
        torch.nn.utils.vector_to_parameters(sampled_params, self.parameters())
        # After prediction, restore original parameters
        def restore_params():
            torch.nn.utils.vector_to_parameters(original_params, self.parameters())
        self._restore_params = restore_params

    def on_train_end(self):
        """Compute the Hessian diagonal for Laplace approximation after training."""
        if self.use_laplace:
