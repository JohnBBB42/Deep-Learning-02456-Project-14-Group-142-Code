...
class LitSWAGPaiNNModel(LitPaiNNModel):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=self.init_lr)
        # We start with an exponential decay scheduler.
        # Once swa_start is reached, we will stop stepping the scheduler
        # and switch to a constant LR by directly setting the optimizer's LR.
        lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.9999996)
